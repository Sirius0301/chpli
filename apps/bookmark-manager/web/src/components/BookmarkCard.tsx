    ...
    <div className="p-4 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-1">{bookmark.url}</p>
          {bookmark.description && (
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 line-clamp-2">{bookmark.description}</p>
          )}
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
            ...
            {bookmark.last_clicked_at && (
              <span>上次 {new Date(bookmark.last_clicked_at).toLocaleDateString()}</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          ...
          >
            <Star size={16} fill={bookmark.is_required ? 'currentColor' : 'none'} />
          </button>
          <button
