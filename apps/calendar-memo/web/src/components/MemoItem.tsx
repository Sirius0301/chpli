...
export function MemoItem({ memo, isHighlighted = false, compact = false }: MemoItemProps) {
  const { 
    toggleMemoComplete, 
    selectMemo, 
    openDetailPanel,
  ...
  return (
    <div 
      className={cn(
        "memo-item group flex items-start gap-2 px-2 py-1.5 rounded text-sm hover:bg-white hover:shadow-sm transition-all cursor-pointer",
        !compact && "border-l-2",
        memo.completed ? 'opacity-50' : 'opacity-100',
        !compact && memo.priority ? priorityColors[memo.priority] : 'border-l-transparent',
